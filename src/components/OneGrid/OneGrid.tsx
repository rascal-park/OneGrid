...
const OneGrid = forwardRef<OneGridHandle, OneGridProps>(
  (
    ...
    function ensureInternalKey(r: any): any {
      const hasUserKey =
        r[rowKeyField] !== undefined && r[rowKeyField] !== null;
    ...
    const [anchorCell, setAnchorCell] = useState<CellCoord | null>(
      null
    );
    ...
    function commitEdit() {
      ...
      const realIndex = internalRows.findIndex(
    ...
    function handleCellClick(
      ...
      if (e.shiftKey && anchorCell) {
        const result = buildRectSelection(
          anchorCell,
          rowIndex,
          colIndex,
          displayRows,
    ...
    useImperativeHandle(ref, () => ({
      ...
      startEditAt(rowKey, colField) {
        const rIndex = displayRows.findIndex(
          (r) => getRowKey(r) === rowKey
        );
        const cIndex = effectiveColumns.findIndex(
          (c) => c.field === colField
        );
