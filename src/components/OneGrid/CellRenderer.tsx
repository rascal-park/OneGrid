...
const CellRenderer: React.FC<CellRendererProps> = ({
  ...
  const formattedValue =
    column.formatter?.format?.(value, row, rowIndex, colIndex) ?? value;
...
function renderByType(
  ...
  switch (type) {
    ...
    case 'image': {
      ...
      const onClick = rendererProps.onClick as
        | ((p: OneGridRenderParams) => void)
        | undefined;
    ...
    case 'icon': {
      ...
      const position: 'left' | 'right' =
        rendererProps.position === 'right' ? 'right' : 'left';
      const size = rendererProps.size ?? 14;

      const text =
        formattedValue != null ? String(formattedValue) : String(value ?? '');
      ...
      const body = (
        <>
          ...
          {text && (
            <span style={{ margin: iconEl ? '0 4px' : 0 }}>{text}</span>
          )}
    ...
    case 'checkbox': {
      const checkValue =
        rendererProps.checkValue !== undefined
          ? rendererProps.checkValue
          : true;
      const uncheckValue =
        rendererProps.uncheckValue !== undefined
          ? rendererProps.uncheckValue
          : false;

      const current = formattedValue ?? value;
      const checked = current === checkValue;

      const onToggle = rendererProps.onToggle as
        | ((p: OneGridRenderParams & { nextValue: any }) => void)
        | undefined;
    ...
    case 'button': {
      const label =
        formattedValue != null ? formattedValue : value ?? '';
      const onClick = rendererProps.onClick as
        | ((p: OneGridRenderParams) => void)
        | undefined;
    ...
    // 6) 드롭다운 리스트
    case 'dropdown': {
      const options = rendererProps.options as
